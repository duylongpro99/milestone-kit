...
| Path | What |
|---|---|
| `milestone-kit/` | The kit: skills, scripts, hooks, templates. Start with its `README.md` and `USAGE.md`. |
...
~/personal/agent/milestone-kit/milestone-kit/scripts/bootstrap/install ~/code/my-app
cd ~/code/my-app && scripts/bootstrap/check
# workers on Codex instead of Claude Code:
~/personal/agent/milestone-kit/milestone-kit/scripts/bootstrap/install ~/code/my-app --agent codex
```

Herdr is a separate tool that must be on `PATH` for the driver; it is not vendored here. Codex support covers the worker sessions only (`milestone-kit/USAGE.md §8`); the driver is always Claude Code. OpenCode is not supported.
