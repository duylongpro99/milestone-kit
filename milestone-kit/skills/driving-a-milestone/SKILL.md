...
This session becomes the **driver** for one milestone. It never writes code, plans, or docs. It spawns one worker agent session per phase inside the milestone's worktree, hands it a self-contained brief, waits, reads the worker's `handoff.md`, and either continues, relays an owner question, or stops. The owner talks only to the driver, except to click an agent dialog in a pane.

**Agents.** `MS_AGENT` in `scripts/milestone/config` (`claude`, default, or `codex`) decides which CLI the workers run; the scripts read it, the driver never guesses. It sets the hooks file (`.claude/settings.json` or `.codex/hooks.json`) and the session skill behind `.claude/skills/cris-managed-session` (`cc-session` or `cx-session`). Two words below are agent-neutral: an **owner question** is `AskUserQuestion` on Claude Code and a plain numbered question on Codex; **in the background** is `run_in_background` on Claude Code and a background shell on Codex.
...
5. Claude Code only (`MS_AGENT=claude`): `.claude/settings.local.json` exists in the root checkout with the owner's allowlist (git, gh, and the project's build/test tools). Without it, every worker commit is a permission dialog; say so and let the owner decide before spawning. On Codex the owner's approval policy and sandbox in `~/.codex/config.toml` play this role, and each new worktree path must be trusted once in the pane.
6. `grep -q guard-scope.sh <hooks file>` succeeds (`.claude/settings.json` on Claude Code, `.codex/hooks.json` on Codex) and `scripts/hooks/guard-scope.sh` is executable. Without the hook the scope is only a request; stop and say so.
7. `grep -q require-handoff.sh <hooks file>` succeeds and `scripts/hooks/require-handoff.sh` is executable (the Stop hook that makes a worker finish its handoff after a cut connection). Without it a disconnect always ends at the owner; say so, the owner decides.
...
4. **Send and wait.** `scripts/milestone/wait <M> --brief` in the background; it sends the one-line prompt, then waits through sleeps and socket drops (default deadline 90 min; `caffeinate` keeps the Mac from idle-sleeping meanwhile). Let its completion notification wake you; never poll with sleep. Act on its `RESULT=` line:

| `RESULT=` | driver does |
|---|---|
| `handoff` | step 5 |
| `blocked` | an agent dialog (permission, trust) in the pane: `herdr agent read <PANE> --lines 40`, show the owner, they answer in the pane (`herdr agent focus <PANE>`), then `scripts/milestone/wait <M>` again |
...
7. **Post-finish** (after finish `DONE`). Run `scripts/milestone/next <M>` from the root checkout and relay its lines verbatim, in this order:
   ...
   4. Name the record: `docs/journal/<SLUG>.md` is now on the base branch. Tell the owner once that `scripts/milestone/journal <M> --recheck` (from the root checkout, any time, no worktree) re-runs its Verification table and that the `owner` rows are the numbered steps under its Owner steps. The worktree and everything under `docs/sdd/<M>/` except `progress.md` can go; the journal is what remains.
...
- Moving past a `CONTINUE` or `DONE` handoff with `JOURNAL=missing` or `stale`, or calling execute or finish `DONE` while `owner-steps` is short: the journal is the only record of the milestone that outlives the worktree.
- Writing or editing `docs/journal/<SLUG>.md` yourself, or telling the owner the verification steps in chat instead of pointing at the journal's Owner steps.
