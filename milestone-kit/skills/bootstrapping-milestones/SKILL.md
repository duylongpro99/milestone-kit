...
| 3 process docs | stage 2 ids | `docs/STATUS.md` row for the first milestone (`unclaimed`, Plan cell `docs/plans/<slug>.md`, slug = id + short kebab title); fill `{{…}}` in `docs/plans/README.md`, `docs/adr/README.md` (+ `0000-template.md`); `docs/sdd/README.md` as seeded; `docs/spike-results.md` when the roadmap has a Gate column | Templates; the only judgment is the slug and the test-kind wording taken from arch §9 or the PRD's testing section | none |
